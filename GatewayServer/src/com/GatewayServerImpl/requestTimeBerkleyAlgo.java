...
public class requestTimeBerkleyAlgo implements Runnable {
	...
	public void run() {
		Iterator ite = GwServerInterfaceImpl.processRecord.keySet().iterator();
		...
		if(catageory.equals("GATEWAY") && !id.equals(Const.ID_GATEWAY)){	
		Registry regs = null;
		...
		try {
			GwServerInterfaceImpl.storeTimeStamps.add(stGatewayObj.provideTimeStamp());
			...
			try {
				GwServerInterfaceImpl.storeTimeStamps.add(stSensorObj.provideTimeStamp());
			} catch (NullPointerException | RemoteException e) {
			...
			try {
				GwServerInterfaceImpl.storeTimeStamps.add(stDeviceObj.provideTimeStamp());
			...
			try {
				GwServerInterfaceImpl.storeTimeStamps.add(stDatabaseObj.provideTimeStamp());
			} catch (NullPointerException | RemoteException e) {
