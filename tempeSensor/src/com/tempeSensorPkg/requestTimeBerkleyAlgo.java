...
public class requestTimeBerkleyAlgo implements Runnable {
	...
	public void run() {
		...
		if(catageory.equals("GATEWAY") && !id.equals(Const.ID_SENSOR_MOTION)){	
		Registry regs = null;
		...
		try {
			tempeSensorImpl.storeTimeStamps.add(stGatewayObj.provideTimeStamp());
			...
			try {
				tempeSensorImpl.storeTimeStamps.add(stSensorObj.provideTimeStamp());
			} catch (NullPointerException | RemoteException e) {
			...
			try {
				tempeSensorImpl.storeTimeStamps.add(stDeviceObj.provideTimeStamp());
			...
			try {
				tempeSensorImpl.storeTimeStamps.add(stDatabaseObj.provideTimeStamp());
			} catch (NullPointerException | RemoteException e) {
