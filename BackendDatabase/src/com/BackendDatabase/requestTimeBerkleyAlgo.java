...
public class requestTimeBerkleyAlgo implements Runnable {
	...
	public void run() {
		Iterator ite = BackendDatabaseimpl.processRecord.keySet().iterator();
        ...
        while(ite.hasNext()){
    		...
    		case Const.ID_DEVICE_BULB:
    			...
    			this.ipAddress = BackendDatabaseimpl.processRecord.get(id);
    			break;
		...
		try {
			BackendDatabaseimpl.storeTimeStamps.add(stGatewayObj.provideTimeStamp());
			...
			try {
				BackendDatabaseimpl.storeTimeStamps.add(stSensorObj.provideTimeStamp());
			} catch (NullPointerException | RemoteException e) {
			...
			try {
				BackendDatabaseimpl.storeTimeStamps.add(stDeviceObj.provideTimeStamp());
