...
public class requestTimeBerkleyAlgo implements Runnable {
	...
	public void run() {
		Iterator ite = bulbSmart.processRecord.keySet().iterator();
        ...
        while(ite.hasNext()){
    		...
    		case Const.ID_DEVICE_BULB:
    			...
    			this.ipAddress = bulbSmart.processRecord.get(id);
    			break;
		...
		try {
			bulbSmart.storeTimeStamps.add(stGatewayObj.provideTimeStamp());
			...
			try {
				bulbSmart.storeTimeStamps.add(stSensorObj.provideTimeStamp());
			} catch (NullPointerException | RemoteException e) {

			}	
		}
		else if(catageory.equals("DEVICE") && !id.equals(Const.ID_DEVICE_BULB)){
			Registry regs = null;
			try {
				regs = LocateRegistry.getRegistry(ipAddress, port);
		} catch (RemoteException e) {
			...
			try {
				bulbSmart.storeTimeStamps.add(stDeviceObj.provideTimeStamp());
			...
			try {
				bulbSmart.storeTimeStamps.add(stDatabaseObj.provideTimeStamp());
			} catch (NullPointerException | RemoteException e) {
