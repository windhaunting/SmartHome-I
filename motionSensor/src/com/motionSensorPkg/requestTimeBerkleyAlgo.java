...
public class requestTimeBerkleyAlgo implements Runnable {
	...
	public void run() {
		Iterator ite = motionSensorImpl.processRecord.keySet().iterator();
        ...
        while(ite.hasNext()){
    		...
    		case Const.ID_DEVICE_BULB:
    			...
    			this.ipAddress = motionSensorImpl.processRecord.get(id);
    			break;
		...
		try {
			motionSensorImpl.storeTimeStamps.add(stGatewayObj.provideTimeStamp());
		...
		else if(catageory.equals("SENSOR") && !id.equals(Const.ID_SENSOR_MOTION)){
			...
			try {
				motionSensorImpl.storeTimeStamps.add(stSensorObj.provideTimeStamp());
			} catch (NullPointerException | RemoteException e) {
			...
			try {
				motionSensorImpl.storeTimeStamps.add(stDeviceObj.provideTimeStamp());
			...
			try {
				motionSensorImpl.storeTimeStamps.add(stDatabaseObj.provideTimeStamp());
			} catch (NullPointerException | RemoteException e) {
