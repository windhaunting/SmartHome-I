...
public class requestTimeBerkleyAlgo implements Runnable {
	...
	public void run() {
		Iterator ite = doorSensorImpl.processRecord.keySet().iterator();
        ...
        while(ite.hasNext()){
    		...
    		case Const.ID_DEVICE_BULB:
    			...
    			this.ipAddress = doorSensorImpl.processRecord.get(id);
    			break;
		...
		try {
			doorSensorImpl.storeTimeStamps.add(stGatewayObj.provideTimeStamp());
		...
		else if(catageory.equals("SENSOR") && !id.equals(Const.ID_DOOR)){
			...
			try {
				doorSensorImpl.storeTimeStamps.add(stSensorObj.provideTimeStamp());
			} catch (NullPointerException | RemoteException e) {
			...
			try {
				doorSensorImpl.storeTimeStamps.add(stDeviceObj.provideTimeStamp());
			...
			try {
				doorSensorImpl.storeTimeStamps.add(stDatabaseObj.provideTimeStamp());
			} catch (NullPointerException | RemoteException e) {
